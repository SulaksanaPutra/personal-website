...
watch(
  ...
  () => {
    ...
    const savedState = localStorage.getItem('contactDrawerOpen')
    if (savedState !== null) {
      isDrawerOpen.value = savedState === 'true'
    } else {
      isDrawerOpen.value = true
      localStorage.setItem('contactDrawerOpen', 'true')
    }
  },
  { immediate: true }
)
...
const toggleDrawer = () => {
  ...
  if (window.innerWidth >= 768) {
    localStorage.setItem('contactDrawerOpen', isDrawerOpen.value.toString())
  }
}
